# Техническое задание: AI CLI Gateway Service

## 1. Общее описание

**Название проекта:** AI CLI Gateway Service  
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_USER: ${POSTGRES_USER:-ai_gateway}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}
    volumes:
      - postgres-data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-ai_gateway}"]
      interval: 5s
      timeout: 5s
      retries: 5
  ...
  ai-gateway:
    ...
    environment:
      ...
      - DATABASE_URL=postgresql+asyncpg://${POSTGRES_USER:-ai_gateway}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-ai_gateway}
...
- `claude mcp add ai-gateway --transport sse http://localhost:8000/api/mcp/sse` регистрирует сервер без ошибок;
...
- `docker compose up` поднимает postgres + ai-gateway + nginx без ошибок;
...
```bash
claude mcp add ai-gateway --transport sse http://localhost:8000/api/mcp/sse
```
