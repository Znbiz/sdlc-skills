...
from app.workflows.init_arch.graph import _route_after_node, build_graph, compile_graph
from app.workflows.init_arch.state import InitArchState


def _make_state(**kwargs) -> InitArchState:
    base = InitArchState(
        product_name="P", analysis_scope="full", workspace_dir="/w", arch_repo_dir="/w/arch",
        engine_name="claude", timeout_seconds=60, progress_file_path="/w/arch/p.yaml",
        current_step_id="define_scope", current_repo_name="", completed_steps=[],
        repo_list=[], domain_strategy="", open_questions=[], answered_questions=[],
        pending_user_question="", last_cli_output="", last_guard_output="",
        step_error=None, retry_count=0,
    )
...
def test_build_graph_has_all_nodes():
    ...
    expected_nodes = [
        "define_scope", "request_repository_list", "prepare_temp_workspace",
        "clone_repositories", "refresh_main_branches", "plan_repository_order",
        "assess_scope_and_domains", "analyze_repositories", "interview_user",
        "refine_features", "build_navigation_index", "run_knowledge_lint",
        "validate_final", "finalize_progress", "handle_error",
    ]
...
async def test_get_checkpointer_returns_cached():
    ...
    try:
        ...
        with patch("app.workflows.init_arch.checkpointer.asyncpg.connect", new_callable=AsyncMock) as mock_connect, \
