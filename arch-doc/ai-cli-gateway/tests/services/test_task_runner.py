...
from app.services.task_runner import _build_cmd, _is_auth_error, cancel_cli_task, run_cli_task, set_db_enabled
...
class TestRunCliTask:
    ...
    async def test_timeout_sets_failed_status(self) -> None:
        ...
        with (
            ...
            unittest.mock.patch("asyncio.wait_for", side_effect=TimeoutError),
        ):
            await run_cli_task(cli_task, pool)
...
class TestDbPersistence:
    ...
    async def test_run_cli_task_calls_upsert_when_db_enabled(self) -> None:
        ...
        with unittest.mock.patch("asyncio.create_subprocess_exec", return_value=mock_proc), \
             ...
             unittest.mock.patch("app.services.task_runner.upsert_cli_task", new_callable=unittest.mock.AsyncMock) as mock_upsert:
            await run_cli_task(cli_task, pool)
