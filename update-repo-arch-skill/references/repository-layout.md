...
├── release-notes/                   # Один файл на каждый завершённый прогон update-repo-arch-skill
│   └── <YYYY-MM-DD>.md              # Дата прогона и ссылки на обновлённые файлы (см. assets/release-note-template.md)
...
| `release-notes/<YYYY-MM-DD>.md` | `assets/release-note-template.md` | После каждого завершённого прогона `update-repo-arch-skill` (шаг `finalize_update`) — один новый файл на прогон, не дополнение старого |
...
4. По мере появления фактов выноси интеграции в `architecture/integrations/`.
5. По мере появления подтверждений собирай контракты в `architecture/contracts/`.
6. По мере появления структуры данных описывай хранилища в `architecture/storage/`.
7. По мере появления новых capability создавай или уточняй документы в `features/`.
8. После прохода по доступным репозиториям провалидируй согласованность артефактов между собой и с исходными репозиториями.
9. Затем собери список открытых вопросов и пробелов.
10. Затем уточни пробелы у пользователя.
11. Перед завершением еще раз провалидируй итоговую картину.
