...
import { useConversation, useConversationItems, useCreateInitArchResponse } from "./hooks";
        ...
        <>
          <Card title="Текущий статус">
            <div className={styles.statusRow}>
              ...
              />
              <span>Шаг: {activeResponse.current_step_id || "—"}</span>
              <span>Репозиторий: {activeResponse.current_repo_name || "—"}</span>
          ...
          {activeResponse.response_status !== "success" && activeResponse.response_status !== "failed" && (
            <Card title="Поток событий">
              {streamState.entries.length === 0 ? (
                ...
                <pre className={styles.streamLog}>{streamState.entries.map((entry) => entry.message).join("\n")}</pre>
              )}
            </Card>
          )}
