...
4. Обновить `architecture/security.md` со следующими обязательными секциями (без них файл не пройдёт автоматическую проверку на шаге `run_knowledge_lint`):
...
2. Добавить в `architecture/risks.md` секцию `## Категории` (обязательная секция, без неё файл не пройдёт автоматическую проверку на шаге `run_knowledge_lint`) со списком найденных рисков.
...
- На своём уровне: если роль/право в `roles-and-permissions.md`, механизм в `security.md`, риск в `risks.md` или operability-заметка описывались файлом из «Удалённые пути» текущего окна — убрать соответствующую запись из нужного артефакта, а не оставлять её как всё ещё действующую.
