...
5. Добавить записи в `architecture/tech-stack.md` со следующими обязательными секциями (без них файл не пройдёт автоматическую проверку на шаге `run_knowledge_lint`):
   - `## Правила заполнения`
   - `## Таблица` — колонки: сервис, зависимость, версия, источник.
   - `## Примечания`
...
- На своём уровне: если зависимость/технология в `tech-stack.md` была удалена из манифеста (пакет убран из `pyproject.toml`/`package.json`/`go.mod`, файл манифеста сам входит в «Удалённые пути») — убрать соответствующую запись, а не оставлять её как всё ещё используемую.
