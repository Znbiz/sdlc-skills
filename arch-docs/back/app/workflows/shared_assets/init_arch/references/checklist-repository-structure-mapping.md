...
- На своём уровне: если в `architecture/structure/<repo>.yml` есть запись про категорию, чьи `paths` целиком совпадают с «Удалённые пути» текущего окна — обнови её (`not_found`/убери путь), а не оставляй указывать на несуществующий код.
