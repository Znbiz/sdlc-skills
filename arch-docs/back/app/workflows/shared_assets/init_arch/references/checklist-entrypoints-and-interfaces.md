...
- На своём уровне: если route/handler/consumer, упомянутый в `architecture/hld.md` или в трассировке какой-либо фичи, входит в «Удалённые пути» текущего окна — убрать его оттуда (или, если это единственная точка входа фичи, понизить статус реализации), а не оставлять ссылку на удалённый код.
