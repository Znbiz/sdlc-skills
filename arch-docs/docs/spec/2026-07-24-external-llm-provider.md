...
- Health-check/тестовый запрос к endpoint при создании подключения в Setup.
...
- Backend: `just test-db-up` + `uv run --extra dev python -m pytest` — **783 passed, 1 xfailed**
  (полный сьют, включая новые файлы: `test_llm_provider_credentials.py`, `test_llm_providers.py`,
  `test_llm_providers.py` (REST), плюс дополненные `test_task_runner.py`, `test_execute.py`,
  `test_init_arch_workflow.py`, `test_llm_worker.py`). Регрессий нет.
- Backend: `uv run --extra dev ruff check .` и `ruff format --check .` — чисто для всех файлов
  этой фичи. Единственная оставшаяся ошибка (`E501` в `app/db/workflow_repo.py:323`) — подтверждена
  как предсуществующая (воспроизводится и на `git stash` без изменений этой фичи), не относится к
  этой работе.
- Frontend: `npx vitest run` — **99 passed** (15 файлов, включая новые `llm-providers-card.test.tsx`
  и дополненные `init-arch-form.test.tsx`/`setup-page.test.tsx`). Регрессий нет.
- Frontend: `npx tsc --noEmit` — чисто, ошибок типов нет.
...
- Docker: backend и frontend пересобраны (`docker compose build`) и перезапущены
  (`docker compose up -d`) после применения миграции `0009` — см. финальный отчёт в конце
  диалога/PR.
...
- `app/api/rest/llm_providers.py` (новый) — REST-ручки.
