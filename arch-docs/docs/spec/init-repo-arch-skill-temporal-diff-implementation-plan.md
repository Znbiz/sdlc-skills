...
### Этап 9. [x] Расширить тестовый контур и регрессии
...
**Мини-отчёт:**
- перед реализацией проведён аудит существующего покрытия (через subagent-исследование) по 12 конкретным сценариям из чеклиста этапа. Результат: 7 сценариев уже полностью закрыты предыдущими этапами (baseline первого/последующих окон, `no_changes`, `invalid_range`, prompt enrichment, quality gate failure — всё это уже тестировалось начиная с Этапов 2-4, 6), но обнаружено 5 реальных пробелов, на которых сфокусирован этот этап;
- **главный системный пробел, обнаруженный аудитом:** весь diff-парсинг (`collect_changed_paths`/rename/delete semantics) в `arch-docs` тестировался только через **мокнутый** `_run_git_command` с вручную написанной строкой вида `"M\tapp/service.py\nR100\told.py\tnew.py\nD\tlegacy.py"` — ни одного теста, который реально запускает `git diff --name-status` на настоящем git-репозитории и проверяет, что парсер (`collect_changed_paths`) корректно обрабатывает *реальный* вывод git. То же самое для legacy `analysis_guard`: `_create_git_repo_with_commits()` создавал только один файл с модификациями, ни разу не переименовывал и не удалял файл;
- в `arch-docs/tests/workflows/init_arch/test_historical.py` добавлен `test_temporal_delta_pipeline_against_real_git_repo_with_multi_commit_rename_and_delete` — реальный git-репозиторий (`tmp_path`, без единого mock), 4 коммита в одном окне (baseline + 3 коммита), включает `git mv` (реальный rename) и `git rm` (реальный delete); тест прогоняет `build_commit_range()` → `collect_changed_paths()`/`collect_diff_summary()`/`collect_commit_log_summary()` последовательно и проверяет результат против фактического вывода git, а не мока: `changed_paths == ["file_b_renamed.py"]`, `renamed_paths == ["file_b.py -> file_b_renamed.py"]`, `deleted_paths == ["file_a.py"]`, ровно 3 строки в commit log (baseline-коммит не входит в диапазон);
- в `arch-docs/tests/workflows/init_arch/test_nodes.py` добавлен `test_node_plan_repository_order_blocks_when_checkout_done_but_temporal_delta_missing` — единственный по-настоящему workflow-level (а не domain-level) тест на quality gate: в отличие от существующих gate-тестов, которые вызывают `advance_step()`/`historical_prep_is_complete()` напрямую как unit-тесты домена, этот тест прогоняет **реальный** `InitArchGuardService` (не мокнутый) через **реальный** `node_plan_repository_order()`, конструируя сценарий "checkout выполнен (`analysis_target_commit_status=CHECKED_OUT`), но temporal delta не собрана до конца (`commit_range_status=RANGE_RESOLVED`, не `DIFF_COLLECTED`)" для non-first окна — и подтверждает, что нода не продвигает workflow, а возвращает `step_error` с сообщением `"historical prep must be completed before this step"`;
- в `init-repo-arch-skill/tests/test_analysis_guard_knowledge.py` добавлен `test_build_temporal_delta_parses_real_rename_and_delete_across_multiple_commits` в `TemporalDeltaUnitTests` — аналогичный real-git сценарий (4 коммита, `git mv`, `git rm`) для legacy CLI, вызывающий `_build_temporal_delta()` напрямую;
- **этот тест сразу поймал реальный баг**, а не гипотетический: `_parse_name_status()` в legacy `analysis_guard/commands.py` (написан на Этапе 8) не добавлял новый путь переименованного файла в `changed_paths`, в отличие от `collect_changed_paths()` в `arch-docs`, которая это делает — расхождение контракта между сервисным и legacy-реализациями, введённое мной же на предыдущем этапе и незамеченное, потому что Этап 8 тестировался только на модификациях файла (`payload.txt`), без единого rename-кейса. Исправлено в `commands.py::_parse_name_status()` — теперь оба парсера ведут себя идентично (renamed target path добавляется в `changed_paths`), что напрямую подтверждает ценность требования плана "расширить git-fixtures: rename/delete cases" — без реального rename-фикстура эта регрессия осталась бы незамеченной;
- существующий `test_timeline_plan_resolve_checkout_and_advance_window` (Этап 8) уже содержит двухоконный сценарий с разными датами коммитов до/после snapshot для двух репозиториев одновременно — доп. требование "разные даты для старых и новых изменений" сочтено покрытым без дублирования;
- `arch-docs/tests/workflows/init_arch/domain/test_operations.py` не потребовал новых тестов: аудит подтвердил, что domain-level инварианты (`historical_prep_is_complete()`, `_repository_temporal_window_is_valid()`) уже полностью покрыты с Этапа 4, включая специфичный баг с прежним строгим `bool(commit_range and ...)` для `no_changes`; добавление ещё одного workflow-level теста в `test_nodes.py` было содержательно более ценным, чем дублирование domain-level проверок;
- прогнаны: `pytest tests/` в `arch-docs` — **362 passed** (было 360, +2 новых теста; `--cov-report=term-missing` подтвердил, что оба новых теста реально исполняют целевые ветки `historical.py`/`nodes.py`); `python3 -m unittest init-repo-arch-skill.tests.test_analysis_guard_knowledge` — **14 passed** (было 13, +1); `ruff check` на изменённые файлы `arch-docs` — 0 новых замечаний после форматирования docstring (D205); для `init-repo-arch-skill` ruff не применялся намеренно (см. мини-отчёт Этапа 8 — у этого репозитория нет собственного lint-конфига, и прогон через конфиг `arch-docs` вносит посторонние auto-fix правки в несвязанные файлы).

### Этап 10. [x] Документация, аудит и критерии эксплуатации
...
**Мини-отчёт:**
- `docs/workflows/init.md`, секция `## Sequence Diagram`: первая (historical prep) diagram расширена — вместо только snapshot-резолва теперь показывает полный temporal-delta pipeline внутри `resolve_target_commits`: `resolve_temporal_baseline` → `build_commit_range` (с ancestor-check) → ветвление на `BASELINE_MISSING`/`NO_CHANGES`/`INVALID_RANGE`/`DIFF_COLLECTED` с соответствующими `TEMPORAL_*` audit-событиями на каждой ветке, и явную Note о том, что gate требует snapshot commit **и** непустой diff-статус, даже если checkout уже выполнен;
- добавлена новая (четвёртая) sequence diagram, покрывающая два ранее не задокументированных потока: diff signal routing внутри `analyze_repositories` (`classify_diff_severity` → `route_checklist_items` → `DIFF_SIGNAL_ROUTED` событие → routed-подмножество чеклиста) и полный `temporal_window_confirmation` loop-back (`compute_next_window` → terminal/pause developing → `interrupt()` → resume с `continue_to_next_window`/`finish_temporal_analysis` → реальный переход графа обратно на `REFRESH_MAIN_BRANCHES` либо на `FINALIZE_PROGRESS`) — до этого этапа loop-back был описан только текстом в разделе "Temporal Window Confirmation", без диаграммы;
- audit trail для temporal delta документирован явно в новом абзаце `## Regression Coverage`: перечислены конкретные real-git regression-тесты (Stage 9) с указанием, какие git-команды и сценарии они реально исполняют (не мокают), включая тест, который поймал реальный баг парности между `arch-docs` и legacy CLI;
- добавлена новая секция `## Operational Guide` с тремя подразделами, дословно закрывающими пункты плана: "Что делать при `invalid_range`" (диагностика через `git log --oneline --all`, объяснение, почему это блокирующее состояние, и рабочий обход через сброс baseline), "Как интерпретировать `baseline_missing`" (два легитимных случая — первое окно без истории и репозиторий, впервые попавший в scope не на первом окне — и то, что worker в этом случае получает только snapshot-state с явной пометкой не доверять diff), "Когда допустим fallback к snapshot-only анализу" (формальный ответ — практически никогда для не-первых окон благодаря Stage 4 gate; единственный легитимный случай — сам `baseline_missing`, который уже explicit и auditable, а не тихий fallback);
- добавлена новая секция `## Граница с update-repo-arch-skill`, дословно закрывающая оба буллета плана: `update-repo-arch-skill` — baseline-to-HEAD delta для уже существующего архитектурного репозитория (один diff на весь интервал с момента последнего обновления документации); `init_arch` — window-to-window diff semantics внутри первичной исторической реконструкции репозитория "с нуля" (последовательность diff-срезов между соседними temporal snapshot'ами, разделённых обязательным user confirmation); явно зафиксировано, какие primitives переиспользуются намеренно (commit_range/diff summaries/signal routing — `route_checklist_items()` в `init_arch` прямо назван адаптацией `checklist-signal-routing.md` из `update-repo-arch-skill`), а что должно оставаться раздельным (domain state, quality gates, workflow orchestration);
- `## Known Gaps` пункт 1 переписан: вместо построчного перечисления "что открыто по этапам" теперь фиксирует, что весь 10-этапный план закрыт целиком, со ссылками на все документированные секции контракта;
- документация не потребовала code-изменений — весь мини-отчёт этого этапа состоит из правок `docs/workflows/init.md`; проверка синтаксической валидности новых mermaid-диаграмм выполнена вручную (балансировка `alt`/`else`/`end`, отсутствие незакрытых блоков) — автоматического mermaid-линтера в проекте нет.
...
**Статус:** все критерии выполнены (Этапы 1-10 закрыты). `commit_range`/`diff_stat_summary`/`commit_log_summary`/`changed_paths` хранятся per-window (Этап 2-3); `historical_prep_is_complete()` блокирует content-анализ без построенной delta (Этап 4); `confirm_next_temporal_window` — реальный graph loop-back с обязательной паузой, доступный через REST и OpenAI-compatible facade (Этап 5); `build_step_prompt()` включает temporal-delta блок (Этап 6); regression-тесты используют real-git репозитории, а не только мокнутый `subprocess.run` (Этап 9); `docs/workflows/init.md` — единый источник правды по diff-aware contract, sequence flow, audit trail, операторским сценариям и границе с `update-repo-arch-skill` (Этап 10).
